...
def eval_vit(loader: Iterable[Tuple[jnp.ndarray, jnp.ndarray]],
             params: flax.core.frozen_dict.FrozenDict,
             eval_batch_fn: Callable) -> Dict[str, float]:
    ...
    for (images, labels) in loader:
        batch_loss, correct, total = eval_batch_fn(params, 
                                                   jnp.array(images), 
                                                   jnp.array(labels))
...
def eval_mlm_task(loader: Iterable[Tuple[torch.Tensor, torch.Tensor]],
                  model: nn.Module,
                  mask_token_id: int) -> Dict[str, float]:
    model.eval()
    total_masks = 0
    correct_preds = 0
    loss = 0.0
    ...
    metric_dict = { "correct_preds": correct_preds,
...
def eval_na_task(loader: Iterable[Tuple[Dict[str, torch.Tensor], torch.Tensor, torch.Tensor]],
                 model: nn.Module,
                 mask_token_id: int) -> Dict[str, float]:
    ...
    for i, (input, correct_ans_ids, incorrect_ans_ids) in enumerate(loader):
        with torch.no_grad():
            ...
            correct_logits = output_logits[torch.arange(output_logits.size(0)), correct_ans_ids]
            incorrect_logits = output_logits[torch.arange(output_logits.size(0)), incorrect_ans_ids]
...
class ModelWeightClipper(BaseParamModifier):
    ...
    def __init__(self, model, epsilon):
        ...
        self.eps = epsilon
        self.orig_params = [(module, name, module._parameters[name].data.detach().clone().cpu()) 
    ...
    def __call__(self):
        with torch.no_grad():
            for module, name, orig_param in self.orig_params:
                ...
                lower_bounds = torch.min(clamp_bound-self.eps*(clamp_bound+1), 
                                         clamp_bound+self.eps*(clamp_bound+1))
                upper_bounds = torch.max(clamp_bound-self.eps*(clamp_bound+1), 
                                         clamp_bound+self.eps*(clamp_bound+1))
...
class MaskGrads(BaseParamModifier):
    ...
    def __init__(self, model: nn.Module, num_random_dirs: int):
        ...
        total_dirs = sum([module._parameters[name].numel() for (module, name) in self.params])
        
        if self.num_random_dirs>total_dirs:
            raise ValueError(f"Number of random directions: {self.num_random_dirs} \
                is greater than total number of parameters: {total_dirs}. Kindly \
                    provide smaller random directions.")
        
        self.random_dirs = random.Random(42).sample(range(total_dirs), self.num_random_dirs)
    ...
    def construct_masks(self):
        ...
        for (module, name) in self.params:
            ...
            while (idx<len(self.random_dirs) and 
                   self.random_dirs[idx]<elems_till_now+num_elems_in_param):
...
def flatness_measure(loader: Iterable[Tuple[Any, Any]],
                     model: nn.Module, 
                     criterion: Union[nn.Module, Callable],
                     optimizer: torch.optim.Optimizer,
                     gradient_accumulation_steps: int=1,
                     epsilon: float=1e-5,
                     num_random_dirs: Optional[int]=None) -> float:
...
def train_epoch(loader: Iterable[Tuple[Any, Any]], 
                model: nn.Module, 
                criterion: Union[nn.Module, Callable], 
                optimizer: torch.optim.Optimizer,
                metric: Optional[datasets.Metric]=None) -> Dict[str, float]:
...
def train_epoch_volume(loader: Iterable[Tuple[Any, Any]], 
                       model: nn.Module, 
                       criterion: Union[nn.Module, Callable], 
                       optimizer: torch.optim.Optimizer,
                       vol_reg: float, nsample: int,
                       metric: Optional[datasets.Metric]=None) -> Dict[str, float]:
...
def train_epoch_multi_sample(loader: Iterable[Tuple[Any, Any]], 
                             model: nn.Module, 
                             criterion: nn.Module, 
                             optimizer: torch.optim.Optimizer, 
                             nsample: int,
                             metric: Optional[datasets.Metric]=None) -> Dict[str, int]:
...
def train_transformer_epoch(loader: Iterable[Tuple[Any, Any]], 
