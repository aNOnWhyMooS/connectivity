#Similar to VGG16 in vgg.py, but for a PreResNet, though.
...
from simplex_models import Conv2d as SimplexConv
from simplex_models import BatchNorm2d as SimplexBN
...
def conv3x3simplex(in_planes, out_planes, fix_points, stride=1):
    return SimplexConv(in_planes, out_planes, kernel_size=3, fix_points=fix_points, stride=stride,
                         padding=1, bias=False)
...
class BasicBlockSimplex(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, fix_points, stride=1, downsample=None):
        super(BasicBlockSimplex, self).__init__()
        self.bn1 = SimplexBN(inplanes, fix_points=fix_points)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = conv3x3simplex(inplanes, planes, stride=stride, fix_points=fix_points)
        self.bn2 = SimplexBN(planes, fix_points=fix_points)
        self.conv2 = conv3x3simplex(planes, planes, fix_points=fix_points)
        self.downsample = downsample
        self.stride = stride
...
class BottleneckSimplex(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, fix_points, stride=1, downsample=None):
        super(BottleneckSimplex, self).__init__()
        self.bn1 = SimplexBN(inplanes, fix_points=fix_points)
        self.conv1 = SimplexConv(inplanes, planes, kernel_size=1, bias=False,
                                   fix_points=fix_points)
        self.bn2 = SimplexBN(planes, fix_points=fix_points)
        self.conv2 = SimplexConv(planes, planes, kernel_size=3, stride=stride,
                                   padding=1, bias=False, fix_points=fix_points)
        self.bn3 = SimplexBN(planes, fix_points=fix_points)
        self.conv3 = SimplexConv(planes, planes * 4, kernel_size=1, bias=False,
                                   fix_points=fix_points)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class PreResNetSimplex(nn.Module):

    def __init__(self, num_classes, fix_points, depth=110):
        super(PreResNetSimplex, self).__init__()
        if depth >= 44:
            assert (depth - 2) % 9 == 0, 'depth should be 9n+2'
            n = (depth - 2) // 9
            block = BottleneckSimplex
        else:
            assert (depth - 2) % 6 == 0, 'depth should be 6n+2'
            n = (depth - 2) // 6
            block = BasicBlockSimplex

        self.inplanes = 16
        self.conv1 = SimplexConv(3, 16, kernel_size=3, padding=1,
        ...
        self.bn = SimplexBN(64 * block.expansion, fix_points=fix_points)
        self.relu = nn.ReLU(inplace=True)
        self.avgpool = nn.AvgPool2d(8)
        self.fc = SimplexLinear(64 * block.expansion, num_classes, fix_points=fix_points)

        for m in self.modules():
            if isinstance(m, SimplexConv):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                for i in range(m.num_bends):
                    getattr(m, 'weight_%d' % i).data.normal_(0, math.sqrt(2. / n))
            elif isinstance(m, SimplexBN):
                for i in range(m.num_bends):
    ...
    def _make_layer(self, block, planes, blocks, fix_points, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = SimplexConv(self.inplanes, planes * block.expansion, kernel_size=1,
