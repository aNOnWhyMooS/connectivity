...
class Linear(SimplexModule):
    ...
    def forward(self, input: torch.Tensor, 
                coeffs_t: Iterable[float]) -> torch.Tensor:
...
class SimplexNet(Module):
    ...
    """
    def __init__(self, n_output: int, architecture: Type[nn.Module], 
                 n_vert: int, fix_points: List[bool]=None,
    ...
    def weights(self, t) -> np.ndarray:
        ...
        for module in self.simplex_modules:
            weights.extend([w for w in module.compute_weights_t(coeffs_t) if w is not None])           #Should raise error if w is None?
