...
def main(args):
    ...
    if args.num_steps is not None:
        save_dir = os.path.join(
            save_dir, "num_steps_" + args.num_steps.replace(",", "_")
        )
    ...
    if glob.glob(save_file_prefix + "*"):
        print(
            f"Found pre-existing checkpoint with prefix {save_file_prefix}. Loading it."
        )
        args.load_pretrained = save_file_prefix
    elif args.begin_from_dataloader != 0:
        raise FileNotFoundError(
            f"Unexpected. Trying to start from {args.begin_from_dataloader}-th"
    ...
    trainloaders, testloader = get_train_test_loaders(
        args,
        tokenizer,
        mnli_label_dict,
    ...
    if args.load_pretrained is not None:
        model_path = (
            ...
            + ("0" if is_fthr and int(model_ids[0]) < 10 else "")
            + model_ids[0]
        )
        ...
        for ii, id in enumerate(model_ids):
            model_path = (
                args.base_models_prefix + ("0" if is_fthr and int(id) < 10 else "") + id
            )
            if num_steps is not None:
                model = get_sequence_classification_model(
                    model_path, num_steps=num_steps[ii]
                )
            else:
                model = get_sequence_classification_model(model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "If specified, the training will being from the n-th train dataloader."
    ...
    if args.begin_from_dataloader > args.break_epochs:
        raise ValueError(
            f"The dataloader to begin with ({args.begin_from_dataloader})"
