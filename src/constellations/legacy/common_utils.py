...
def get_imgData_loader(
    args, resolution, *get_img_dataset_args, **get_img_dataset_kwargs
):
    ...
    dataset = dataset.map(
        lambda e: {
            "data_image": pp(e["img"], resolution),
    ...
    dl = DataLoader(dataset, batch_size=args.batch_size, drop_last=True)
    ...
    class loader_wrapper:
        ...
        def __iter__(self):
            for batch in self.dl:
                ...
                images = images.reshape(images.shape[0], resolution, resolution, 3)
...
def get_NA_loader(args, *get_ml_dataset_args, **get_ml_dataset_kwargs):
    ...
    def get_ids(tokens):
        ...
        if ids[0] == tokenizer.unk_token_id and ids[1] == tokenizer.unk_token_id:
            raise ValueError(
                f"At least one of correct/incorrect answers must \
                be in the vocab. Both {tokens[0]} and {tokens[1]} are not in the vocab."
            )
        return {"correct_ans_id": ids[0], "incorrect_ans_id": ids[1]}

    dataset = dataset.map(lambda e: get_ids([e["correct_ans"], e["incorrect_ans"]]))

    dataset = dataset.map(
        lambda e: tokenizer(
            e["sentence"], truncation=True, padding="max_length", return_tensors="pt"
        )
    )
...
def get_logits_converter(
    mnli_label_dict: Dict[str, int], hans_label_dict: Dict[str, int], pool="max"
) -> Callable:
    ...
    def mnli_logits_to_hans(model_out: SCO) -> SCO:
        ...
        contradiction_logits = model_out["logits"][
            ..., mnli_label_dict["contradiction"]
        ]
        ...
        if pool == "max":
            hans_logits[..., hans_label_dict["non-entailment"]] = torch.max(
                contradiction_logits, neutral_logits
            )
        elif pool == "sum":
            hans_logits[..., hans_label_dict["non-entailment"]] = (
                contradiction_logits + neutral_logits
            )
        else:
...
def select_revision(path_or_name, num_steps: int, local_dir=None, tmp_dir=None):
    ...
    if local_dir is not None:
        ...
        repo = Repository(
            local_dir=tmp_dir, clone_from=path_or_name, skip_lfs_files=True
        )
        repo = Repo(tmp_dir)
    ...
    **select_revision_kwargs,
    ...
    if len(select_revision_kwargs) != 0:
        if os.path.isdir(path_or_name):
            raise AssertionError(
                f"To load older commits, must fetch model from HuggingFace remote url repo,\
    ...
    try:
        model = model_type.from_pretrained(
            path_or_name, from_flax=from_flax, revision=revision
        )
...
get_sequence_classification_model = functools.partial(
    get_model, model_type=AutoModelForSequenceClassification
)
...
def expand_resolution(params, resolution: int, patch_size: int, model_config):
    posemb = params["Transformer"]["posembed_input"][
        "pos_embedding"
    ]  # Shape [1, 1+{res*res/(patch_size*patch_size)}, d_emb]
    ...
    if resolution != orig_resolution:
        ...
        posemb_grid = posemb_grid.reshape(
            n_patches_per_axis_old, n_patches_per_axis_old, -1
        )
        posemb_grid = scipy.ndimage.zoom(posemb_grid, zoom, order=1)
        posemb_grid = posemb_grid.reshape(
            1, n_patches_per_axis * n_patches_per_axis, -1
        )
...
def get_criterion_fn(
    ...
    logit_converter_fn: Optional[Callable[[SCO], SCO]] = None,
) -> Callable[[SCO, torch.Tensor], torch.Tensor]:
...
def get_pred_fn(
    pred_type: str = "argmax",
    logit_converter_fn: Optional[Callable[[SCO], SCO]] = None,
) -> Callable[[SCO], torch.Tensor]:
    ...
    if pred_type not in supported_pred_types:
        raise NotImplementedError(
            f"Prediction function for doing {pred_type} type predictions not implemented.\
