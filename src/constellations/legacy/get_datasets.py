...
def get_cola_data(split: str, n_parts: int = 1, index: int = 0) -> datasets.Dataset:
    ...
    if split == "in_domain_dev":
        ...
        raise ValueError(
            f"Unknown split: {split}. Use one of [in_domain_dev, out_domain_dev,\
...
def get_mnli_data(
    ...
    if split not in ["train", "validation_matched"]:
        raise ValueError(
            f"Unknown split: {split}. Only 'train' and 'test'(=='validation_matched') \
    ...
    if labels_dict is not None:
        for k in labels_dict.keys():
            if k not in ["contradiction", "entailment", "neutral"]:
                raise ValueError(
                    "Invalid label: {}, found in label dict. \
                                  Only 'contradiction', 'entailment' and \
                                  'neutral' are valid".format(
                        k
                    )
                )

        original_labels = {
            elem: i for i, elem in enumerate(dataset.info.features["label"].names)
        }
...
def get_hans_data(
    ...
    if split not in ["train", "validation"]:
        raise ValueError(
            f"Unknown split: {split}. Only 'train' and 'test'(=='validation') \
                         are supported currently."
        )
    ...
    if heuristic_wise is not None:
        ...
        if len(set(heuristic_wise).intersection(set(available_heuristics))) < len(
            heuristic_wise
        ):
            raise ValueError(
                "Invalid element in heuristic_wise. Can't find one or more \
                              heuristics in the HANS dataset."
            )
        dataset = dataset.filter(lambda e: e["heuristic"] in heuristic_wise)

    if onlyNonEntailing:
        non_entailment_idx = dataset.info.features["label"].names.index(
            "non-entailment"
        )
        if non_entailment_idx == -1:
            raise AssertionError(
                "Can't find non-entailment category in HANS dataset.info, check install."
            )
...
def get_ml_dataset(
    ...
    if data_path is None:
        ...
        data_path = os.path.join(
            os.getcwd(), "bert-syntax", "marvin_linzen_dataset.tsv"
        )
    dataset = load_dataset(
        "csv",
        data_files={"test": "bert-syntax/marvin_linzen_dataset.tsv"},
        delimiter="\t",
        column_names=["case", "subcase", "correct_sent", "incorrect_sent"],
    )["test"]
    ...
    dataset = dataset.map(
        lambda e: add_masks(e["correct_sent"], e["incorrect_sent"])
    ).filter(lambda e: e["sentence"] is not None)

    if cases is not None:
        datasets = [
            dataset.filter(
                lambda e: (
                    e["case"] == case and (subcases is None or e["subcase"] in subcases)
                )
            )
...
def get_mlm_data(
    dataset: str, shuffle: bool = True, n_parts: int = 1, index: int = 0
) -> datasets.Dataset:
...
def get_cifar100_dataset(
    split: str = "test", shuffle: bool = True, n_parts: int = 1, index: int = 0
) -> datasets.Dataset:
...
def get_img_dataset(
    ...
    if dataset == "cifar100":
        ...
        if split != "test":
            raise ValueError(
                f"Only test split is available for imagenet-a, \
                but {split} split was specified. "
            )

        return get_imagenet_dataset(
            adversarial=True, shuffle=shuffle, n_parts=n_parts, index=index
        )
    else:
        raise ValueError(
            f"Unsupported dataset: {dataset}. Only cifar100, \
...
class ECE:
    ...
    def add_batch(self, predictions: torch.Tensor, references: torch.Tensor) -> None:
        ...
        if not (labels.shape[0] == predictions.shape[0] == references.shape[0]):
            raise AssertionError(
                "Number of predictions, references and labels must be equal."
            )

        for label, probs, ref in zip(labels, predictions, references):
            pred_label_prob = probs[label]
            bin_no = int(pred_label_prob * self.n_bins) - (
                1 if pred_label_prob == 1.0 else 0
            )
    ...
    def compute(self) -> Dict[str, int]:
        ...
        """
        confidences = [
            0 if n_samples_in_bin == 0 else tot_prob_in_bin / n_samples_in_bin
            for tot_prob_in_bin, n_samples_in_bin in zip(
                self.tot_probs, self.samples_per_bin
            )
        ]

        accuracies = [
            0 if n_samples_in_bin == 0 else correct_samples_in_bin / n_samples_in_bin
            for correct_samples_in_bin, n_samples_in_bin in zip(
                self.correct_samples, self.samples_per_bin
            )
        ]
        ...
        ece = (
            sum(
                [
                    n_samples_in_bin * abs(acc - conf)
                    for (n_samples_in_bin, acc, conf) in zip(
                        self.samples_per_bin, accuracies, confidences
                    )
                ]
            )
            / total_samples
