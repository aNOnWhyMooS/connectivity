    ...
    **select_revision_kwargs,
    ...
    try:
        if ret_type == "pt":
            ...
            model = model_type.from_pretrained(
                path_or_name, from_pt=(not from_flax), revision=revision
            )
        else:
        ...
        model = model_type.from_pretrained(
            base_model,
            num_labels=params["classifier.weight"].shape[0],
        )
    ...
    if wrap_forward:
        assert (
            ret_type == "pt"
        ), "forward method only in pt models, so can only wrap them"
...
get_flax_seq_classification_model = functools.partial(
    get_model,
    ...
    if only_digit_suffix:
        ...
        for model in models:
            if re.fullmatch(r"\d+", model.modelId[len(prefix) :]) is not None:
                ...
                warnings.warn(
                    f"Skipping model with non-digit suffix \
