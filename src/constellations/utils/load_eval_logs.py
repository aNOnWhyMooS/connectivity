...
def convert_dataset_to_dict(dataset: datasets.Dataset) -> Dict[str, Dict[str, Any]]:
    ...
    for sample in dataset:
        if "premise" in sample:
            ...
            new_dataset[key] = {k:sample[k] for k in sample if k not in ["premise", "hypothesis"]}
        elif "sentence" in sample:
            key = make_dict_key(sample["sentence"])
            new_dataset[key] = {k:sample[k] for k in sample if k not in ["sentence"]}
        else:
            raise NotImplementedError(f"Unknown sample type. Unable to find \
...
def adjust_labels(dataset: datasets.Dataset, 
                  data_dict: Dict[str, Dict[str, Any]], 
                  data_dict_labels: Dict[str, int]) -> Dict[str, Dict[str, Any]]:
...
def get_select_heuristic(hans_dataset: Dict[str, Dict[str, Any]], 
                         heuristic: str, 
                         onlyNonEntailing: bool=False,
                         entailment_idx: int=None) -> Callable:
    ...
    def select_heuristic(sample_dict):
        ...
        if label!=sample_dict["label"]:
            raise AssertionError(f"Found different labels: {label} and {sample_dict['label']}")
...
def convert_pred_list_to_dict(
    pred_list: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
...
def get_avg_metrics(predictions_list, glue_subset="mnli"):
    total_loss = reduce(lambda carry, sample: carry+sample["loss"], 
                        predictions_list, 0)
    avg_loss = total_loss/len(predictions_list)
    
    metric = load_metric("glue", glue_subset)
    for sample in predictions_list:
        metric.add(prediction=sample["probabilities"].index(max(sample["probabilities"])),
...
def get_nli_model_metrics(filename: str, 
    ...
    if heuristic is not None and entailing is not None:
        preds = list(filter(lambda pred_dict: ((hans_data_dict[pred_dict["sentence"]]["heuristic"]==heuristic) and 
                                               (entailing==(pred_dict["label"]==entailment_index))
                                               ), preds))
    
    return get_avg_metrics(list(map(add_losses, preds)))

def get_cola_model_metrics(filename: str,
                           split: str,):
    eval_log = read_file(filename)
    preds = eval_log["samples"]
...
def get_qqp_model_metrics(filename: str,):
    eval_log = read_file(filename)
    preds = eval_log["samples"]
...
def natural_order(lis: List[str]) -> List[str]:
    ...
    def get_key(word):
        ...
        if len(digits)!=1:
            warnings.warn(f"Unable to find one single group of digits in\
...
def get_metrics(directory: str,
                heuristic: Optional[str]=None, 
                entailing: Optional[bool]=None) -> Dict[str, Tuple[float, float]]:
    
    files = [os.path.join(directory, elem) for elem in os.listdir(directory) 
    ...
    return {model : get_nli_model_metrics(models_to_files[model], heuristic, entailing) 
            for model in sorted_model_names}


def get_metrics_with_sufs(prefix: str,
                          suffixes: List[str],
                          heuristic: Optional[str]=None, 
                          entailing: Optional[bool]=None,
                          split: Optional[str]="validation",
                          data_type:Optional[str]="nli") -> Dict[str, Tuple[float, float]]:
    
    files = [prefix+suffix+("" if suffix.endswith(".json") else ".json") 
                for suffix in suffixes]
    
    if data_type=="nli":
        return {suffix : get_nli_model_metrics(file, heuristic, entailing) 
                for (suffix, file) in zip(suffixes, files)}
    elif data_type=="qqp":
        return {suffix : get_qqp_model_metrics(file) 
                for (suffix, file) in zip(suffixes, files)}
    elif data_type=="cola":
        return {suffix : get_cola_model_metrics(file, split)
                for (suffix, file) in zip(suffixes, files)}
