...
def get_interpolation_wise_data(lines):
    ...
    for line in lines:
        match_obj = re.match(
            r"Euclidean distance between (.+) and (.+): (\d+\.\d+).", line
        )
...
def get_data_from_strs(interpol_strs, idx=1):
    match_obj = re.match(
        r"Euclidean distance between (.+) and (.+): (\d+\.\d+).", interpol_strs[0]
    )
    ...
    if len(loss_vals) != 10:
        print(interpol_strs)
        raise ValueError(
            f"Number of loss values read, must be 10! Instead read: {loss_vals}\
...
def load_logs(
    directory, metric="loss"
) -> Dict[Tuple[str, str], Tuple[float, List[float]]]:
    """
    Returns:
