...
def eval(loader, model, criterion, binary=False, coeffs_t=None):
    ...
    for i, input in enumerate(loader):
        ...
        with torch.no_grad():
            output = model(
                input_ids, attention_mask, type_ids, target, coeffs_t=coeffs_t
            )
            loss = output.loss
        ...
        if binary:
            ...
            with torch.no_grad():
                ...
                x = torch.max(
                    torch.concat(
                        (logits[:, 0].view(-1, 1), logits[:, 2].view(-1, 1)), 1
                    ),
                    1,
                ).values
...
def train_epoch_multi_sample(loader, model, criterion, optimizer, nsample):
    loss_sum = 0.0
    correct = 0.0
...
def train_transformer_epoch(
    ...
    for i, input in enumerate(tqdm(loader)):
        ...
        if wandb and i % 20 == 0:
            wandb.log(
                {"simplex_loss": loss.item(), "num_correct": correct, "volume": vol}
            )
            wandb.watch(model)
