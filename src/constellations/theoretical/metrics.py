...
def inner_mod_BH_metric(euc_dist, losses):
    ...
    for i in range(1, 9):
        ...
        while right_min<9 and losses[right_min+1]<=losses[right_min]:
            right_min += 1
        linear_interpol_val = (losses[right_min]-losses[left_min])*(bump_center-left_min)/(right_min-left_min)+losses[left_min]
...
def inner_mod_mod_BH_metric(euc_dist, losses):
    ...
    for i in range(1, 9):
        ...
        if min_left_loss>losses[bump_center] or min_right_loss>losses[bump_center]:
            ...
            min_right_idx = losses[bump_center+1:].index(min_right_loss)+bump_center+1
            linear_interpol_val = (min_right_loss-min_left_loss)*(bump_center-min_left_idx)/(min_right_idx-min_left_idx)+min_left_loss
            bump_height = losses[bump_center]-linear_interpol_val
            if bump_height>=max_height:
...
def BaH_metric(euc_dist, losses):
    ...
    for i in range(1, len(losses)-1):
        new_diff = losses[i]-((losses[0]*((len(losses)-1)-i)+losses[-1]*i)/(len(losses)-1))
...
def model_similarity(pred_dict1, pred_dict2):
    ...
    for sent_pair in pred_dict1["samples"]:
        same_preds += int((pred_dict1["samples"][sent_pair]["correct"]
...
def functional_similarity(pred_dict1, pred_dict2):
    ...
    shift_labels_dict = {v: pred_dict1["label_dict"][k] 
                         for k,v in pred_dict2["label_dict"].items()}
    same_preds = 0
    for sent_pair in pred_dict1["samples"]:
        probs1 = reorder_probs(pred_dict1["samples"][sent_pair]["probabilities"],
                               shift_labels_dict)
        probs2 = pred_dict2["samples"][sent_pair]["probabilities"]
        same_preds += int((probs1.index(max(probs1))
                           ==probs2.index(max(probs2))))
