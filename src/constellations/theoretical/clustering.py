...
def get_spectral_embed(dist_matrix: np.ndarray, n_clusters: int=2, delta=1.0,):
    sim_matrix = np.exp(-dist_matrix**2 / (2. * delta ** 2))
    se = spectral_embedding(sim_matrix, 
                            random_state=random_state,
                            n_components=n_clusters-1,)
    return se.reshape(-1)

def get_sc_centroid_dists(dist_matrix: np.ndarray, 
                          n_clusters: int=2, delta=1.0,) -> List[List[float]]:
