...
def main(args):
    ...
    first_point = collections.OrderedDict({k:v.to(device) for k,v in points[0].items()})
    last_point = collections.OrderedDict({k:v.to(device) for k,v in points[-1].items()})
    ...
    for i, coeffs_segment in enumerate(coeffs):
        ...
        for coeffs in coeffs_segment:
            ...
            print(f"Distance of {k}-th model from first model: {dist_from_first_model}", flush=True)
            print(f"Distance of {k}-th model from last model: {dist_from_last_model}", flush=True)
            k+=1
