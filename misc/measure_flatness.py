...
def get_parser():
    parser = argparse.ArgumentParser(description="Measure the ɛ-sharpness of a BERT model.")
    ...
    parser.add_argument("--lr", type=float, default=8e-5, help="initial learning rate (default: 8e-5)",)
    parser.add_argument("--opt", type=str, default="sgd", choices=["sgd", "adam"], help="optimizer to use (default: sgd)",)
    parser.add_argument("--gradient_accumulation_steps", type=int, default=1, help="number of gradient accumulation steps",)
    parser.add_argument("--dataset", type=str, default="mnli", choices=["mnli", "hans"], help="dataset [mnli or hans]",)
    parser.add_argument("--n_batches", type=int, default=10, help="number of batches to use for evaluation and training.",)
    parser.add_argument("--epsilon", type=float, default=1e-5, help="epsilon for the epsilon-sharpness metric")
    parser.add_argument("--num_random_dirs", type=int, required=False, help="If provided, the loss will be maximized in only these many directions.\
                                                                             By default, the loss is maximized in all directions.")
    parser.add_argument("--heuristics", type=str, nargs="+", default=["lexical_overlap", "constituent", "subsequence"], help="heuristics to use for HANS dataset")
    parser.add_argument("--only_non_entailing", action="store_true", help="only use non-entailing examples for HANS")
    return parser
...
def main(args):
    ...
    flatness = simp_utils.flatness_measure(input_target_loader,
                                           model, criterion,
                                           optimizer,
