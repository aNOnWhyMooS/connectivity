...
def clean_model_name(model_name):
    cleaned = (
        "0" if (model_name == "0" or model_name == "00") else model_name.lstrip("0")
    )
    return cleaned
        ...
        for model2 in suf_ordered_models:
            for model3 in suf_ordered_models:
                ...
                if d12 + d23 + 1e-10 >= d13:
                    ...
                    percent_triangle_ineq_diff.append(
                        100 * (d13 - (d12 + d23)) / (d12 + d23 + 1e-10)
                    )
    print("Total triangle inequality violation:", sum(triangle_ineq_diff))
    print(
        "Mean triangle inequality violation:",
        sum(triangle_ineq_diff) / (total_triplets - exact_triangle_ineq),
    )
    print("Exact inequality satisfied for:", exact_triangle_ineq)
    print("Total triplets:", total_triplets)
    print(
        "Mean percentage difference:",
        sum(percent_triangle_ineq_diff) / (total_triplets - exact_triangle_ineq),
    )
