...
def get_accs(args):
    if "_" in str(args.perf_metric):
        ...
        accs = {k: v[1] for k, v in get_metrics_with_sufs(args.eval_mods_prefix, 
                                                          args.eval_mods_suffixes, 
                                                          heuristic, entailing).items()}
    else:
        accs = {k: v[1] for k, v in get_metrics_with_sufs(args.eval_mods_prefix, 
                                                          args.eval_mods_suffixes).items()}
    return accs
...
if __name__=="__main__":
    ...
    flatness = {}
    for line in lines:
        match_obj = re.match(r"ɛ-sharpness of ../../../100_berts/bert_(\d\d) is (\d+\.\d+)", line)
    ...
    print(flatness.keys())
    accs = get_accs(args)
    outs = [accs[("0" if (k<10 and 'feather' in args.eval_mods_prefix) else "")+str(k)] for k in flatness]

    save_file = "epsilon_sharpness_"+args.perf_metric+".pdf"
    lineplot(np.array(inps), np.array(outs), [list(range(len(inps)))], save_file, 
             ylabel=args.perf_metric, xlabel=r"$\epsilon$-sharpness")