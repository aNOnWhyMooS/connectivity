    ...
    parser.add_argument(
        ...
        choices=([elem1+elem2 for (elem1, elem2) in 
                 itertools.product(["subsequence", "lexical_overlap", "constituent"],
                                    ["_onlyNonEntailing"])]+["in_domain_dev", "out_domain_dev"]),
...
def get_dist_matrix(interpol_vals, ordered_models, metric_fn):
    return np.array([[metric_fn(*interpol_vals[(model1, model2)]) 
...
def get_dataframe(peaks, valleys, plains, interpol_losses, interpol_accs, dataset_name):
    ...
    def append_data(lis1, lis2, lis1_type, lis2_type,):
        same=(lis1==lis2)
        for i, model1 in enumerate(lis1):
            for j, model2 in enumerate(lis2):
                if i==j and same:
                    continue
                model1, model2 = (("0" if (model1=="0" or model1=="00") else model1.lstrip("0")), 
                ...
                inter_dict["type"]+=[rename[lis1_type]+"-to-"+rename[lis2_type]]*len(losses)
...
if __name__=="__main__":
    ...
    assert (args.interpol_log_dirs is None or 
            len(args.interpol_datasets)==len(args.interpol_log_dirs))
    ...
    print("Valley models & their accs:", valleys, [accs[i] for i in valleys], flush=True)
    ...
    dfs = []
    for (interpol_dir, dataset_name) in zip(args.interpol_log_dirs, args.interpol_datasets):
        interpol_losses = load_logs(interpol_dir)
        interpol_accs = load_logs(interpol_dir, metric="acc")
        df = get_dataframe(peaks, valleys, plains, interpol_losses, interpol_accs, dataset_name)
        dfs.append(df)
    df = pd.concat(dfs)
    
    save_file = ("_".join(args.interpol_datasets)+"--"
                +args.eval_mods_prefix.replace("/", "_")+"--"
                +("" if args.perf_metric is None 
                     else "perf_metric="+args.perf_metric)).replace("..", "").replace("--", "-")
    
    save_file = ''.join(elem.title() for elem in save_file.split("_"))
    df = df.rename(columns={"dataset_names": "data"})
    plot_plain_valley_peaks(df, save_file+"_losses.pdf", "type", "position", "losses", "data")
    plot_plain_valley_peaks(df, save_file+"_accs.pdf", "type", "position", "accs", "data")