...
if __name__=="__main__":
    ...
    interpol_vals = [load_logs(interpol_log_dir) for interpol_log_dir in args.interpol_log_dirs]
    
    suf_ordered_models = list(time_wise_accs[0].keys())
    
    dist_matrices =[get_dist_matrix(interpols, suf_ordered_models, metric_fn) for interpols in interpol_vals]
    ...
    time_wise_clusters = [get_clusters(dist_matrix, args.n_clusters) for dist_matrix in dist_matrices]
        
    time_wise_clusters = [[[suf_ordered_models[idx] for idx in cluster] 
    ...
    plot_quiver(dist_matrices, time_wise_accs, time_wise_clusters, times, 
                quiver_save_file, ylabel, eval_metric=args.eval_metric)