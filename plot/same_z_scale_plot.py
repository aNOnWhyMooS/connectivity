...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Plot multiple surfaces, with same Z-axis scale."
    )
    parser.add_argument(
        "--surface_pkl_files",
        nargs="+",
    ...
    parser.add_argument(
        "--names", nargs="+", type=str, default=None, help="Names of the surfaces."
    )
    parser.add_argument(
        "--savename",
        type=str,
        default="temp",
        help="File where the final plot is to be saved.",
    )
    parser.add_argument(
        "--clip_x",
        type=float,
        nargs=2,
        help="Will clip graph to be within these x values.",
    )
    parser.add_argument(
        "--clip_y",
        type=float,
        nargs=2,
        help="Will clip graph to be within these y values.",
    )
    parser.add_argument(
        "--clip_z",
        type=float,
        nargs=2,
        help="Will clip graph to be within these z values.",
    )
    parser.add_argument(
        "--points_per_level",
        type=int,
        default=20,
        help="How many points to put in same level?",
    )
    parser.add_argument(
        "--point_names",
        nargs="+",
        help="Names for points in order of surface_pkl_files",
    )
    return parser
...
def plot_loss_surface(
    ...
    def plot_locations(ax, locations, point_name_loc):
        ...
        print(
            "Renamed Locations:",
            point_names[point_name_loc : point_name_loc + len(locations)],
        )
    ...
    fig, axs = plt.subplots(
        nrows=1, ncols=len(surfaces), figsize=(8 * len(surfaces), 8)
    )
    ...
    xlims = (
        max([e[0] for e in xlimss] + ([clip_x1] if clip_x1 is not None else [])),
        min([e[1] for e in xlimss] + ([clip_x2] if clip_x2 is not None else [])),
    )
    ylims = (
        max([e[0] for e in ylimss] + ([clip_y1] if clip_y1 is not None else [])),
    ...
    for surface, locations in surfaces:
        ...
        index = np.logical_and(
            np.logical_and(xx >= xlims[0], xx <= xlims[1]),
    ...
    for i, ((surface, locations), ax) in enumerate(zip(surfaces, axs)):
        ...
        ax.imshow(f, cmap="coolwarm", extent=list(xlims + ylims))
