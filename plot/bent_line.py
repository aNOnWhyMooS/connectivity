...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "File having logs measuring performance on the non-linear path."
            "All files must correspond to same path."
        ),
    )

    parser.add_argument("--metric", default="loss", type=str, help="Metric to plot.")
...
def get_dists(lines: List[str]) -> List[float]:
    for line in lines:
        match_obj = re.fullmatch(r"Distances between points on bent line: (.*)", line)
        if match_obj is not None:
            return ast.literal_eval(match_obj.group(1))
    raise AssertionError(
        "Couldn't find a line starting" " mentioning the distances between lines."
    )
...
def get_indices(lines: List[str]) -> Tuple[int, int]:
    for line in lines:
        ...
        if match_obj is None:
            raise AssertionError(
                f"The base model prefix: {prefix}" f"doesn't have indices."
            )
...
def get_steps(lines: List[str]) -> Tuple[int, int]:
    for line in lines:
        ...
        if match_obj is None:
            raise AssertionError(
                f"The base model prefix: {prefix}" f"doesn't have num_steps."
            )
        return (int(match_obj.group(1)), int(match_obj.group(2)))
...
def segment_to_dict(
    lines: List[str], fields: List[str], metric: Optional[str] = None
) -> List[Dict[str, float]]:
...
def parse_data(lines: List[str], metric: str) -> Tuple[str, List[List[float]]]:
    ...
    segment_dicts = map(
        partial(segment_to_dict, fields=fields, metric=metric), segment_wise_lines
    )
    segment_wise_metrics = [[e[metric] for e in segment] for segment in segment_dicts]
...
def check_files(args) -> Optional[Tuple[Tuple[int, int], Tuple[int, int]]]:
    ...
    for f in args.log_files:
        ...
        if not (
            (prev_steps is None or prev_steps == steps)
            and (prev_indices is None or prev_indices == indices)
        ):
...
def get_linear(metric, steps, indices):
    ...
    interpol_logs = load_logs(
        ("../../conss/logs/NLI/long_qqp_berts/" f"qqp_interpol@{steps[0]}steps/"),
        metric=metric,
    )
...
def plot_linear(
    args, steps: Tuple[int, int], indices: Tuple[int, int], tot_points=None
):
...
def main(args):
    ...
    if out is not None:
        ...
        plt.title(
            f"Bent path between model {indices[0]}@{steps[0]} steps"
