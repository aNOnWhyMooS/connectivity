...
if __name__ == "__main__":
    ...
    ordered_model_indices = [
        suf_ordered_models.index(model) for model in ordered_models
    ]
    ...
    save_file = (
        (
            args.interpol_log_dir.replace("/", "_").replace(".", "")
            + "--"
    ...
    heatmap_save_file = (
        "interpolate="
        + args.interpol_log_dir.strip("/").split("/")[-1]
        + "_perf_metric="
        + str(args.perf_metric)
        + ".pdf"
    )
