...
from constellations.model_loaders.load_model import (
    get_sequence_classification_model,
    get_flax_seq_classification_model,
)
...
def main(args):
    is_feather_bert = ("feather" in args.models[0]) or ("feather" in args.models[1])
    ...
    if args.do_perm:
        ...
        m1, m2 = match_params(
            m1, m2, model_type=("roberta" if "roberta" in args.base_model else "bert")
        )
    ...
    euclidean_dist = torch.sqrt(
        sum(
            [
                ...
                for (_, v1), (_, v2) in zip(w1.items(), w2.items())
            ]
        )
    ).item()
    print(
        f"Euclidean distance between {args.models[0]}@{args.steps[0]} and {args.models[1]}@{args.steps[1]}: {euclidean_dist}.",
        flush=True,
    )
    print(
        f"Interpolating between {args.models[0]}@{args.steps[0]} and {args.models[1]}@{args.steps[1]} on {args.dataset}",
        flush=True,
    )
    ...
    for k in range(args.n_sample + 2):
        ...
        metrics = eval(input_target_loader, model, criterion, pred_fn, metric)
        if k == 0:
            columns += [key for key in metrics.keys() if key != "loss"]

        values = [k, metrics["loss"]] + [
            v for key, v in metrics.items() if key != "loss"
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of file where to save the interpolation values," "using pickle.",
    )
    ...
    print(
        f"Completed interpolation from {args.models[0]} to {args.models[1]}", flush=True
    )
