...
# File taken at commit eab3381, Jan 27, 2022 from
...
class ModelArguments:
    ...
    data_shuff_seed: Optional[int] = field(
        ...
        metadata={
            "help": (
                "If specified, this seed will be used to shuffle data. Otherwise, --seed will be used."
    ...
    def __post_init__(self):
        if (
            self.another_init_seed
            and not self.another_seed_coeff
            or not self.another_init_seed
            and self.another_seed_coeff
        ):
            raise AssertionError(
                "Both another_init_seed and another_seed_coeff"
...
class DataTrainingArguments:
    ...
    only_first_n: Optional[int] = field(
        default=None,
        metadata={"help": "Will only train for first n steps if specified."},
    )
...
def reinit_model(
    ...
    if model_args.another_init_seed:
        if "seed" not in model_init_args:
            logger.warning(
                "Taking combination of a model head initialised"
                " without seed and another with seed."
            )
        model_init_args["seed"] = model_args.another_init_seed
        model1 = FlaxAutoModelForSequenceClassification.from_pretrained(
            **model_init_args
        )
        coeff = model_args.another_seed_coeff
        model.params = jax.tree_util.tree_map(
            lambda x, y: coeff * x + (1 - coeff) * y, model1.params, model.params
        )
    return model
...
def main():
        ...
        metrics = jax.lax.pmean(
            {
                ...
                "grad_norm": pytree_norm(grad),
                "first_moment_norm": pytree_norm(state.opt_state[1][0].mu),
                "second_moment_norm": pytree_norm(state.opt_state[1][0].nu),
            },
            axis_name="batch",
        )
    ...
    def eval_step(state, batch, labels):
        ...
        return state.logits_fn(logits), loss
    ...
    if data_args.task_name is not None:
        ...
        metric = load_metric("accuracy", experiment_id=exp_id)
    ...
    if jax.process_index() == 0:
        ...
        if training_args.push_to_hub:
            repo.push_to_hub(
                commit_message=f"Saving weights and logs of step 0", blocking=False
            )
        ...
        for step, batch in enumerate(
            ...
            if (
                ...
                for batch in tqdm(
                    ...
                    predictions, losses = p_eval_step(state, batch, labels)

                    eval_loss += training_args.per_device_eval_batch_size * jnp.sum(
                        losses
                    )
                ...
                if num_leftover_samples > 0 and jax.process_index() == 0:
                    ...
                    labels = batch.pop("labels")
                    predictions, losses = eval_step(unreplicate(state), batch, labels)

                    eval_loss += num_leftover_samples * jnp.sum(losses)
                    num_eval_samples += num_leftover_samples

                    metric.add_batch(predictions=predictions, references=labels)

                eval_metric = metric.compute()
                eval_metric["loss"] = eval_loss / num_eval_samples
