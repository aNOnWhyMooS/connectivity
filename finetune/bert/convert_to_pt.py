    ...
    for name, array in zip(names, arrays):
        ...
        for m_name in name:
            if re.fullmatch(r"[A-Za-z]+_\d+", m_name):
                ...
                scope_names = [m_name]
            if (
                scope_names[0] == "output_weights"
                or scope_names[0] == "output_bias"
...
if __name__ == "__main__":
    ...
    tokenizer.push_to_hub(
        hf_repo_dir, commit_message="Saving tokenizer", use_auth_token=hf_auth_token
    )
    ...
    for file in os.listdir(model_save_dir):
        ...
        if os.path.isfile(filepath) and "events.out.tfevents" in filepath:
            api.create_commit(
                ...
                operations=[
                    CommitOperationAdd(
                        path_in_repo=filepath.split("/")[-1], path_or_fileobj=filepath
                    )
                ],
